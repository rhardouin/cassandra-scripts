...
Example::
    ...
    ./vnodes_token_generator.py --json --indent 2 --servers hosts_interleaved_racks.txt 4
    {
...
def show_cass_yaml(tokens_per_srv):
    ...
    for index_or_hostname, srv_tokens in tokens_per_srv.items():
        print("%s initial_token: %s" % (index_or_hostname, ','.join(srv_tokens)))
...
def show_text(tokens_per_srv):
    """Print space separated tokens

    :param tokens_per_srv: dict key=server number/hostname, value=its tokens
    """
    for index_or_hostname, srv_tokens in tokens_per_srv.items():
        print("%s %s" % (index_or_hostname, ' '.join(srv_tokens)))
...
if __name__ == '__main__':
    ...
    format_group.add_argument('-y', '--yaml', action='store_true',
                              help='initial_token for cassandra.yaml')
    format_group.add_argument('-t', '--text', action='store_true',
                              help='Space separated values. First column: IP '
                                   'address, then one column per token')
